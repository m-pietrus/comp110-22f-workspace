...
def emojified(guess: str, wordle: str) -> str:
    """This will check contains_char function, and replace True or False with its corresponding Emoji."""
    output: str = ""
    idx: int = 0
    ...
    while idx < len(wordle):
        if guess[idx] == wordle[idx]:
            ...
            if contains_char(wordle, guess[idx]) is True:
...
def input_guess(length: int,) -> str:
    """To ensure that the inputed guess is the same length as the wordle."""
